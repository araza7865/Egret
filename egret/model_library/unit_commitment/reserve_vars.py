...
def garver_power_avail_vars(model):
    ...
    ###################Added by Ahsan###################################
    def maximum_power_avaiable_reliable_expr_rule(m, g, t):
        if g.find('T2') == -1 and g.find('T3') == -1: 
            return m.MaximumPowerAvailableAboveMinimum[g,t] + m.MinimumPowerOutput[g,t]*m.UnitOn[g,t]
        else:
            return 0
    
    model.MaximumPowerAvailableReliable = Expression(model.ThermalGenerators, model.TimePeriods, rule=maximum_power_avaiable_reliable_expr_rule)
